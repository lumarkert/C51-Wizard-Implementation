from training_environment.game_real_evaluation.real_evaluation_card import RealEvaluationCard
from training_environment.game_real_evaluation.real_evaluation_trick import RealEvaluationTrick


class RealEvaluationRound:
    ...
    def determine_card(self, card_string):
        if len(card_string) < 2:
            return None
        color = card_string[0]
        if not color.isalpha() or (
                color != "y" and color != "r" and color != "b" and color != "g" and color != "w" and color != "j"):
            return None
    ...
    def play_trick(self, trick_number):
        self.current_trick = RealEvaluationTrick(self.players, self.start_player, self.trump_suit, trick_number + 1, self.round_number, self.deck)
