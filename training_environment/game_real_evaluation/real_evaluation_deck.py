    ...
    def rebuild_deck(self):
        ...
        for idx, color in enumerate(self.colors):
            for value in range(1, self.number_of_values + 1):
                self.cards.append(RealEvaluationCard(color, value, card_id, idx + 1))
                card_id += 1
        for _ in range(self.number_of_wizards):
            self.cards.append(RealEvaluationCard("NOC", 15, card_id, 0))
            card_id += 1
        for _ in range(self.number_of_jesters):
            self.cards.append(RealEvaluationCard("NOC", 14, card_id, 0))
            card_id += 1
