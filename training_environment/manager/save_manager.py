...
def save_player_data_tournament(directory, players):
    data_dir = os.path.join(directory, "tournament")
    ...
    values = []
    for player in players:
        values.append(player.avg_accuracies[0])
    ...
    with open(os.path.join(data_dir, f'avg_accuracy_csv_{player.name}.csv'), 'w', encoding="ISO-8859-1",
              newline='') as f:
        write = csv.writer(f)
        write.writerow(['Names', 'Average Accuracy'])
        write.writerows(export_data)

    values = []
    for player in players:
        values.append(player.avg_points[0])
    ...
    with open(os.path.join(data_dir, f'avg_points_csv_{player.name}.csv'), 'w', encoding="ISO-8859-1",
              newline='') as f:
        write = csv.writer(f)
        write.writerow(['Names', 'Average Points'])
        write.writerows(export_data)

    values = []
    for player in players:
        values.append(player.winrates[0])

    data = [names, values]
    export_data = zip_longest(*data, fillvalue='')

    with open(os.path.join(data_dir, f'winrate_csv{player.name}.csv'), 'w', encoding="ISO-8859-1",
              newline='') as f:
        write = csv.writer(f)
        write.writerow(['Names', 'Winrate'])
        write.writerows(export_data)

    for player_idx, player in enumerate(players):
        with open(os.path.join(data_dir, f'guessed_tricks_csv_{player.name}_{player_idx}.csv'), 'w',
                  newline='') as f:
            write = csv.writer(f)
            write.writerows(player.history_guessed_tricks_sorted_after_round_number[0])

        with open(os.path.join(data_dir, f'avg_accuracy_per_round_csv_{player.name}_{player_idx}.csv'), 'w',
                  newline='') as f:
            write = csv.writer(f)
            write.writerows(player.avg_accuracy_per_round[0])

        with open(os.path.join(data_dir, f'boxplot_points_csv_{player.name}_{player_idx}.csv'), 'w',
                  newline='') as f:
            write = csv.writer(f)
            write.writerows(player.history_points_sorted_after_round_number[0])

        with open(os.path.join(data_dir, f'avg_points_per_round_csv_{player.name}_{player_idx}.csv'), 'w',
                  newline='') as f:
            write = csv.writer(f)
            write.writerows(player.avg_points_per_round[0])

        with open(os.path.join(data_dir, f'won_tricks_per_round_csv_{player.name}_{player_idx}.csv'), 'w',
                  newline='') as f:
            write = csv.writer(f)
            write.writerows(player.history_won_tricks_sorted_after_round_number[0])


def save_wrapper_data_tournament(wrapper, start_step, training_config):
    ...
    with open(os.path.join(data_dir, f'train_loss_csv_{wrapper.name}.csv'), 'w', encoding="ISO-8859-1",
        ...
        write.writerows(export_data)
def save_files_after_tournament(directory, players):
    save_player_data_tournament(directory, players)
...
def plot_stats_per_round_for_last_round(start_step, game_config, training_config, players):
    ...
    for player_idx, player in enumerate(players):
        for plot_idx, plot_step in enumerate(plot_steps):
            ...
            file_name = f'per_round/avg_accuracy_per_round_step{plot_step}_{player.name}_{player_idx}.png'
            plt.legend()
            ...
            plt.grid()
            file_name = f'per_round/avg_points_per_round_step{plot_step}_{player.name}_{player_idx}.png'
            plt.legend()
            plt.savefig(os.path.join(player.player_directory, file_name))
            plt.clf()
            ...
            file_name = f'per_round/boxplot_points_step{plot_step}_{player.name}_{player_idx}.png'
            plt.savefig(os.path.join(player.player_directory, file_name))
            plt.clf()
...
def determine_save_file_path(name):
    current_dir = os.getcwd()
    current_time = datetime.now().strftime("%H_%M")
    current_date = datetime.now().strftime("%d_%m")
    new_dir = os.path.join(current_dir, f'Tests/{current_date}/{current_time} {name}')
